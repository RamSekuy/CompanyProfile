"use client";

import Card from "@/app/_components/cardComponent/card";
import TestimonialCard from "@/app/_components/cardComponent/testimonialCard";
import Image from "next/image";
import { useState, useEffect } from "react";

export default function GameList() {
  const [data, setData] = useState([]);
  const [isLoading, setLoading] = useState(true);
  const [limit, setLimit] = useState(8);
  const [modal, setModal] = useState({ active: false, data: {} });

  useEffect(() => {
    fetch(`https://pokeapi.deno.dev/pokemon?offset=${limit - 7}&limit=8`)
      .then((res) => res.json())
      .then((fetched) => {
        setData([...data, ...fetched]);
        setLoading(false);
      });
  }, [limit]);

  if (isLoading)
    return (
      <>
        <div className="w-full grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 px-4 my-4">
          {data.map((e, i) => (
            <button
              className="w-max h-max m-auto"
              onClick={() => {
                setModal({ active: true, data: e });
              }}
              key={i}
            >
              <Card data={e} />
            </button>
          ))}
        </div>
        <h1 className="text-center">Loading...</h1>
      </>
    );
  if (!data) return <p>Failed Fetch Data</p>;

  return (
    <>
      <div className="w-full grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 px-4 my-4">
        {data.map((e, i) => (
          <button
            className="w-max h-max m-auto shadow-lg"
            onClick={() => {
              setModal({ active: true, data: e });
            }}
            key={i}
          >
            <Card data={e} />
          </button>
        ))}
      </div>

      <button
        className="p-2 my-4 border-gray-200 bg-green-500 hover:bg-green-600 m-auto"
        onClick={() => {
          setLoading(true);
          setLimit(limit + 10);
        }}
      >
        See more...
      </button>

      {/* modal  */}
      <div
        className={`fixed bg-[rgba(0,0,0,0.5)] z-50 top-0 h-screen right-0 left-0 ${
          !modal.active ? "scale-0" : "scale-100"
        }`}
        onClick={() =>
          setModal({
            active: false,
            data: modal.data,
          })
        }
      >
        {modal.data.stats ? (
          <div
            className={`z-40 w-full h-full p-4 flex flex-col md:flex-row justify-center items-center`}
          >
            {/* Image  */}
            <div
              className={`w-[50%] aspect-square relative transition-all ${
                modal.active
                  ? "translate-y-0 translate-x-0"
                  : "-translate-y-full -translate-x-full"
              }`}
            >
              <Image
                src={modal.data.imageUrl}
                alt="Game Image"
                fill
                sizes="100%"
              />
            </div>

            <div className="w-full md:w-[50%] bg-green-700 border-2 border-green-400 p-5">
              <h1 className="text-4xl font-extrabold text-white">
                {modal.data.name + " Game"}
              </h1>
              {/* Star Rate  */}
              <div
                className={`w-[50%] flex my-4 transition-all delay-300 ${
                  modal.active ? "scale-100 opacity-100" : "scale-0 opacity-0"
                }`}
              >
                {Object.keys(
                  Array.apply(
                    0,
                    Array(
                      modal.data.stats.Attack >= 100
                        ? 5
                        : Math.ceil(modal.data.stats.Attack / 20)
                    )
                  )
                ).map((e, i) => (
                  <div
                    className={`relative w-[20%] aspect-square
                    }`}
                    key={i}
                  >
                    <Image
                      alt="star"
                      fill
                      src={
                        "data:image/png;base64,"
                      }
                    />
                  </div>
                ))}
              </div>

              {/* Customer Comment */}
              <div className="w-full flex gap-2 p-2 items-center overflow-x-auto">
                {[5, 5, 5].map((e, i) => (
                  <TestimonialCard
                    key={i}
                    star={e}
                    customerName={`Customer ` + (i + 1)}
                  />
                ))}
              </div>
            </div>
          </div>
        ) : (
          <></>
        )}
      </div>
    </>
  );
}
